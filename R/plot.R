plotTrajectories = function(x, obj.cols = c("tsp", "wtsp", "ttp"), in.one.plot = TRUE, to01 = FALSE, highlight.which = NULL, highlight.path = FALSE, highlight.start = TRUE, highlight.area = FALSE, ...) {
  ...
  if (is.null(x$objective.type)) {
    ...
    g = ggplot2::ggplot(dfl, aes(x = iter, y = Value))
    g = g + ggplot2::geom_line(aes(color = as.factor(objective.type), linetype = as.factor(objective.type)))
    g = g + ggplot2::theme_bw()
    g = g + ggplot2::scale_color_brewer(palette = "Dark2")
    g = g + ggplot2::labs(
      x = "Iteration",
      ...
      color = "EA driver",
      linetype = "EA driver")
    g = g + facet_wrap(. ~ Objective, scales = "free_y")
    g = g + ggplot2::theme(legend.position = "top")
    return(g)
  }
}
